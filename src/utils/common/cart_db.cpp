...
static struct cart_db_entry homebrew_entry = {"ED", "EJP", CART_DB_SAVE_TYPE_NONE,
                                              "Homebrew ROM with header in EverDrive / Libdragon format"};
...
const struct cart_db_entry *cart_db_get_entry(const uint8_t *rom)
{
    ...
    for (i = 0; i < NUM_CART_DB_ENTRIES; i++) {
        if (!strncmp(cart_db_table[i].rom_id, rom_id, 3) && strchr(cart_db_table[i].regions, rom_id[3]))
    ...
    if (rom_id[1] == 'E' && rom_id[2] == 'D') {
        ...
        switch (config >> 4) {
            case 1:
                homebrew_entry.save_type = CART_DB_SAVE_TYPE_EEPROM_4KBIT;
                break;
            case 2:
                homebrew_entry.save_type = CART_DB_SAVE_TYPE_EEPROM_16KBIT;
                break;
            case 3:
                homebrew_entry.save_type = CART_DB_SAVE_TYPE_SRAM_256KBIT;
                break;
            case 4:
                homebrew_entry.save_type = CART_DB_SAVE_TYPE_SRAM_768KBIT;
                break;
            case 5:
                homebrew_entry.save_type = CART_DB_SAVE_TYPE_FLASH_1MBIT;
                break;
            case 6:
                homebrew_entry.save_type = CART_DB_SAVE_TYPE_SRAM_1MBIT;
                break;
        }
...
bool cart_db_is_well_formed(void)
{
    ...
    qsort(cart_db_table_sorted, NUM_CART_DB_ENTRIES, sizeof(*cart_db_table_sorted), cart_db_table_sorter);
    for (i = 0; i < NUM_CART_DB_ENTRIES - 1; i++) {
        if (!strcmp(cart_db_table_sorted[i].rom_id, cart_db_table_sorted[i + 1].rom_id)) {
            return false;
        }
    }
    for (i = 0; i < NUM_CART_DB_ENTRIES; i++) {
