...
#define DD_BM_STATUS_C1ERR    0x00010000U
...
const uint32_t zone_sec_size[16] = {
    232, 216, 208, 192, 176, 160, 144, 128, 216, 208, 192, 176, 160, 144, 128, 112,
};
...
int read_dd_regs(void *opaque, uint32_t address, uint32_t *word)
{
    ...
    enum dd_register      reg    = (dd_register)(offset >> 2);
    *word                        = dd->regs[reg];
...
void dd_update_bm(struct dd_controller *dd)
{
    ...
    if (dd->retail && ((dd->regs[DD_ASIC_CUR_TK] & 0xFFF) == 6) && (dd->start_block == 0)) {
        dd->regs[DD_ASIC_CMD_STATUS] &= ~DD_STATUS_DATA_RQ;
...
{
    uint32_t offset = dd->track_offset + dd->start_block * SECTORS_PER_BLOCK * zone_sec_size[dd->zone] +
...
void set_offset(struct dd_controller *dd)
{
    ...
    uint16_t              tr_off           = 0;
    static const uint32_t start_offset[16] = {0x00000000, 0x005F15E0, 0x00B79D00, 0x010801A0, 0x01523720, 0x01963D80,
                                              0x01D414C0, 0x020BBCE0, 0x023196E0, 0x028A1E00, 0x02DF5DC0, 0x03299340,
    ...
    dd->track_offset = start_offset[dd->zone] + tr_off * zone_sec_size[dd->zone] * SECTORS_PER_BLOCK * BLOCKS_PER_TRACK;
}
const struct dd_variant *dd_identify_variant(struct rom_file *ipl)
{
    ...
    static struct dd_variant unknown = {"Unknown",
                                        0xDD,
                                        {
                                            0,
                                        }};
    ...
    if (ipl->size != 0x400000)
        return &unknown;
    sha1((uint8_t *)ipl->ptr, ipl->size, sha1_calc);
